...
func TestPaneTrackingClientOpenNewPaneResolvesTargetsLikeNMode(t *testing.T) {
	t.Parallel()
	...
	if err := client.OpenNewPane([]string{"todo.txt"}); err != nil {
	...
	for _, call := range tmux.calls {
		...
		if got, want := call[8], "exec '/tmp/bin/ion' -A -- '/tmp/work/todo.txt'"; got != want {

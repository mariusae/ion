...
type recursivePickerUpdate struct {
	generation int
	paths      []string
	done       bool
	err        error
...
func (s *filePickerPreviewState) begin(startFileID, startDotStart, startDotEnd int) {
	*s = filePickerPreviewState{
		active:        true,
		startFileID:   startFileID,
		startDotStart: startDotStart,
...
func (s *filePickerPreviewState) syncTargetSelection(svc wire.TermService, token string, apply func(wire.BufferView)) (bool, error) {
	token = strings.TrimSpace(token)
	if !s.active {
		return false, nil
	}
	if !s.messageActive && token != "" && token == s.previewToken {
		return false, nil
	}
	if s.previewTransient {
		if _, _, err := deleteCurrentMenuFile(svc, s.previewFileID, false); err != nil {
	...
	if addr, ok := clienttarget.ParseAddressOnly(token); ok {
		view, err = svc.SetAddress(addr)
	} else {
		menu, menuErr := svc.MenuFiles()
		if menuErr != nil {
			return false, menuErr
		}
		existing := make(map[int]struct{}, len(menu))
		for _, file := range menu {
			existing[file.ID] = struct{}{}
		}
		target := clienttarget.Parse(token)
		view, err = svc.OpenTarget(target.Path, target.Address)
		if err == nil {
			if _, ok := existing[view.ID]; !ok {
				transient = true
			}
		}
	}
...
func (s *filePickerPreviewState) finish(svc wire.TermService, commit bool, item overlayPickerItem, apply func(wire.BufferView), setPrevious func(int)) (bool, error) {
	...
	if s.previewToken != "" && startFileID != 0 {
		view, err := svc.SetDot(s.startDotStart, s.startDotEnd)
		if err != nil {
			return false, err
		}
		apply(view)
...
func shouldPreviewPickToken(token string) (bool, string, error) {
	token = strings.TrimSpace(token)
	if token == "" {
		return false, "[not plumbable]", nil
	}
	...
	if strings.TrimSpace(target.Path) == "" {
		return false, "[not previewable]", nil
	}
...
func runTTY(stdin *os.File, stdout, stderr io.Writer, svc wire.TermService, capture *OutputCapture, options Options) error {
	...
	refreshBuffer = func() error {
		return refreshBufferWithReveal(false)
	}
	...
	syncFilePickerPreview = func() (bool, error) {
		...
		case overlayModeFilePicker, overlayModeDirectoryPicker, overlayModeRecursiveFilePicker, overlayModePickPicker:
		default:
			return false, nil
		}
		item, ok := overlay.pickerSelected()
		...
		if overlay.pickerMode() == overlayModePickPicker {
			token := plumbTargetLineToken(item.value)
			if token == "" {
				applyPreviewMessage(item, "[not plumbable]")
				filePickerPreview.messageActive = true
				filePickerPreview.previewPath = ""
				filePickerPreview.previewToken = ""
				if buffer != nil {
					buffer.status = "?not plumbable"
				}
				return true, nil
			}
			ok, message, err := shouldPreviewPickToken(token)
			if err != nil {
				return false, err
			}
			if !ok {
				applyPreviewMessage(item, message)
				filePickerPreview.messageActive = true
				filePickerPreview.previewPath = ""
			...
			changed, err := filePickerPreview.syncTargetSelection(svc, token, applyBufferView)
			if err != nil {
				applyPreviewMessage(item, "[not previewable]")
				filePickerPreview.messageActive = true
				filePickerPreview.previewPath = ""
				filePickerPreview.previewToken = token
				if buffer != nil {
					buffer.status = "?not previewable"
				}
				return true, nil
			}
			if changed {
		...
		if (overlay.pickerMode() == overlayModeDirectoryPicker || overlay.pickerMode() == overlayModeRecursiveFilePicker) && item.fileID == 0 && strings.TrimSpace(item.path) != "" {
	...
	executeLocalIonCommand := func(line string) (bool, bool, error) {
		...
		if command, ok := splitTerminalPseudoCommandArg(normalized, ":term:pick"); ok {
			...
			if buffer != nil {
				...
				startDotStart = buffer.dotStart
				startDotEnd = buffer.dotEnd
			}
	...
	cancelRecursivePicker := func() {
		if recursivePickerCancel != nil {
			recursivePickerCancel()
			recursivePickerCancel = nil
			recursivePickerGeneration++
		}
	}
	defer cancelRecursivePicker()

	openRecursiveFilePicker := func() error {
		if overlay.visible && overlay.pickerMode() == overlayModeRecursiveFilePicker {
			previewChanged, err := finishFilePickerPreview(false, overlayPickerItem{})
			if err != nil {
				return err
			}
		...
		recursivePickerCancel = cancel
		items, preferred := buildRecursiveFilePickerItems(root, nil, buffer, recursivePickerFiles)
		overlay.openPicker(overlayModeRecursiveFilePicker, items, preferred)
	...
	applyRecursivePickerUpdates := func() error {
		if overlay == nil || overlay.pickerMode() != overlayModeRecursiveFilePicker || overlay.picker == nil {
			cancelRecursivePicker()
			return nil
		}
		changed := false
	...
	toggleOverlayShortcut := func(key int) (bool, error) {
		...
		if overlay.visible {
			...
			} else if overlay.pickerMode() == overlayModeFilePicker || overlay.pickerMode() == overlayModeDirectoryPicker || overlay.pickerMode() == overlayModeRecursiveFilePicker || overlay.pickerMode() == overlayModePickPicker {
	...
	recallLastPicker := func() error {
		...
		if overlay.pickerMode() == overlayModeFilePicker || overlay.pickerMode() == overlayModeDirectoryPicker || overlay.pickerMode() == overlayModeRecursiveFilePicker || overlay.pickerMode() == overlayModePickPicker {
			startFileID := 0
			startDotStart := 0
			startDotEnd := 0
			if buffer != nil {
	...
	handleBufferSpecial := func(key int, mouse *mouseEvent) (bool, error) {
		...
		case metaKey('I'):
			return false, openRecursiveFilePicker()
		case metaKey('r'):
				...
				if overlay.pickerActive() {
					if r == 0x1b {
						...
						case keyFocusOut:
							...
							continue
						case metaKey('j'), metaKey('k'), metaKey('p'), metaKey('i'), metaKey('I'), metaKey('g'), metaKey('r'):
					...
					case '\r', '\n':
						...
						case overlayModeRecursiveFilePicker:
							item, ok := overlay.pickerSelected()
							cancelRecursivePicker()
							overlay.close()
							previewChanged := false
...
func scanRecursiveFilePicker(ctx context.Context, generation int, root string, updates chan<- recursivePickerUpdate, notify func()) {
	...
	lastSend := time.Now()
	ignores := newRecursiveIgnoreMatcher(root)
	send := func(done bool, err error) bool {
		update := recursivePickerUpdate{
	...
	err := filepath.WalkDir(root, func(path string, entry os.DirEntry, walkErr error) error {
		...
		if entry.IsDir() {
			if path != root && (recursivePickerAlwaysSkipDir(entry.Name()) || ignores.ignored(path, true)) {
				return filepath.SkipDir
			}
...
func revealBufferDestination(previous, next *bufferState, overlay *overlayState, allowFileSwitch, force bool) *bufferState {
	if next == nil {
		return next
	}
	if previous != nil {
		if previous.fileID != next.fileID && !allowFileSwitch {
			return next
		}
		if !force && previous.fileID == next.fileID && previous.dotStart == next.dotStart && previous.dotEnd == next.dotEnd {
			return next
		}
	}
...
func revealOverlaySelection(previous, next *bufferState, overlay *overlayState) *bufferState {
	...
	return revealBufferDestination(previous, next, overlay, false, false)
}
